...
Development resolves to `{ url, close() }`. Build and dev commands default to
`pnpm exec astro build|dev`, and a theme can provide shell-free argv arrays in
`tooling.build` and `tooling.dev`.
